...
// IMU预积分约束
EdgeInertial::EdgeInertial(IMU::Preintegrated *pInt):JRg(pInt->JRg.cast<double>()),  //.cast<double> double形式
    ...
    JPa(pInt->JPa.cast<double>()), mpInt(pInt), dt(pInt->dT)
{
    ...
    Matrix9d Info = pInt->C.block<9,9>(0,0).cast<double>().inverse();  // C信息矩阵  .block<9,9>(0,0)表示从矩阵中提取一个大小为9x9的子块，并指定子块的起始位置为(0,0)取协方差
    Info = (Info+Info.transpose())/2; //.transpose()转置
    Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,9,9> > es(Info); //计算对称矩阵特征值和特征向量的类
    Eigen::Matrix<double,9,1> eigs = es.eigenvalues(); //eigenvalues()计算矩阵的特征值
    for(int i=0;i<9;i++)
        if(eigs[i]<1e-12)
            eigs[i]=0;
    Info = es.eigenvectors()*eigs.asDiagonal()*es.eigenvectors().transpose(); //asDiagonal用于将一个向量作为对角线元素创建成一个对角矩阵
    setInformation(Info); //设置协方差矩阵
}
