...
// 所有的MapPoints和关键帧做bundle adjustment优化
...
//* 包含IMU对所有的MapPoints和关键帧做bundle adjustment优化 
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit)
{
    ...
    vector<size_t> vnIndexEdgeMono;
    vector<size_t> vnIndexEdgeStereo;
    vpEdgesMono.reserve(N);  // 为容器分配内存空间
    vpEdgesStereo.reserve(N);
    vnIndexEdgeMono.reserve(N);
    vnIndexEdgeStereo.reserve(N);
...

    {
        ...
        for(int i=0; i<N; i++)
        {
            MapPoint* pMP = pFrame->mvpMapPoints[i]; // mvpMapPoints 立体坐标和深度
            if(pMP)
            {
                cv::KeyPoint kpUn;
                // Left monocular observation
    ...
    // 第二种边（IMU预积分约束）：两帧之间位姿的变换量与IMU预积分的值偏差尽可能小
    ...
    // 第三种边（陀螺仪的随机游走约束）：陀螺仪的随机游走值在相邻帧间不会相差太多 residual=VG-VGk
    EdgeGyroRW* egr = new EdgeGyroRW();
    // 将上一帧的BG加入第三种边
    egr->setVertex(0,VGk);
    // 将当前帧的BG加入第三种边  
    egr->setVertex(1,VG);
    // C值在预积分阶段更新，range(9,12)？对应陀螺仪偏置的协方差，最终为cvInfoG值为inv(∑(GyroRW^2/freq))
    Eigen::Matrix3d InfoG = pFrame->mpImuPreintegrated->C.block<3,3>(9,9).cast<double>().inverse();
    // 设置信息矩阵
    egr->setInformation(InfoG);
    // 把第三边加入优化器
    ...
    // 第四种边（加速计随机游走约束）：加速度的随机游走值在相近帧间不会相差太多  residual=VA-VAk
    EdgeAccRW* ear = new EdgeAccRW();
    //将上一关键帧的BA加入第四种边  
    ear->setVertex(0,VAk);
    //将当前帧的BA加入第四种边  
    ear->setVertex(1,VA);
    // C值在预积分阶段更新，range(12,12)对应加速度偏置的协方差，最终cvInfoG值为inv(∑(AccRW^2/freq))
    Eigen::Matrix3d InfoA = pFrame->mpImuPreintegrated->C.block<3,3>(12,12).cast<double>().inverse();
    //设置信息矩阵  
    ear->setInformation(InfoA);
    //把第四种边加入优化器  
    ...
    for(size_t it=0; it<4; it++)
    {   
        // 初始化优化器，这里的参数0代表只对level为0的边进行优化（不传参数也默认为0）
        optimizer.initializeOptimization(0);
        // 每次优化迭代十次
        optimizer.optimize(its[it]);
        ...
        for(size_t i=0, iend=vpEdgesMono.size(); i<iend; i++)
        {
            EdgeMonoOnlyPose* e = vpEdgesMono[i];  // 位姿
    ...
    // cout << ei->exp_0() << ' ' << ei->exp_1() << ' ' << ei->exp_2() << ' ' << ei->exp_3() << ' ' << ei->exp_4() << ' ' << ei->exp_5() << ' ' << ei->exp_6() << ' ' << ei->exp_7() << ' ' << ei->exp_8() << endl;
    ofstream file("error.txt", std::ofstream::app);
    if(file.is_open()) {
        file << ei->exp_0() << ' ' << ei->exp_1() << ' ' << ei->exp_2() << ' ' << ei->exp_3() << ' ' << ei->exp_4() << ' ' << ei->exp_5() << ' ' << ei->exp_6() << ' ' << ei->exp_7() << ' ' << ei->exp_8() << endl;
        file.close();
    } else {
        cout << "文件打开失败." << std::endl;
    }

    // 若四次优化后内点数小于30，尝试恢复一部分不那么糟糕的坏点
    if ((nInliers<30) && !bRecInit)
    {
        // 重新重0开始统计坏点数
        nBad=0;
    ...
    // Step4：记录当前帧的优化状态，包括参数信息和边缘化后的海森矩阵
