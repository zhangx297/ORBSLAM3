...
{   
    // Load camera parameters from settings file 加载图像参数
    if(settings){
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename)
{
    ...
    {
        if(mbRGB)
            cvtColor(mImGray,mImGray,cv::COLOR_RGB2GRAY); //转为灰度图像
        else
            cvtColor(mImGray,mImGray,cv::COLOR_BGR2GRAY);
    }
    else if(mImGray.channels()==4)
    ...
    {
        //构造Frame类  //左目图像，时间戳，左目特征提取器，字典，相机模型，去畸变参数，基线长度，远点近点区分阈值
...
{
    //mCurrentFrame当前帧，mpPrevFrame指向上一帧的指针
    ...
    mvImuFromLastFrame.clear(); // 清空容器中的所有元素
    mvImuFromLastFrame.reserve(mlQueueImuData.size()); //.reserve预分配元素的存储空间；mlQueueImuData帧之间IMU测量的队列
    ...
    while(true)
    {
        bool bSleep = false;
        {
            unique_lock<mutex>  lock(mMutexImuQueue);   //unique_lock独占所有线程（对mutex）
            if(!mlQueueImuData.empty())
            {
                IMU::Point* m = &mlQueueImuData.front(); //front()返回容器中第一个元素的引用
                cout.precision(17);  //设定输出值以新的浮点数精度值显示
                if(m->t<mCurrentFrame.mpPrevFrame->mTimeStamp-mImuPer)
                {
                    mlQueueImuData.pop_front();  //删除列表容器第一个元素
                }
                else if(m->t<mCurrentFrame.mTimeStamp-mImuPer) 
                {
                    //储存两帧间n次IMU测量
                    mvImuFromLastFrame.push_back(*m); // push_back容器尾部添加数据
    ...
    for(int i=0; i<n; i++)
    {
        ...
        // 开始预积分
        mpImuPreintegratedFromLastKF->IntegrateNewMeasurement(acc,angVel,tstep); //从上一关键帧
        pImuPreintegratedFromLastFrame->IntegrateNewMeasurement(acc,angVel,tstep); //上一帧到当前帧的预积分
    }
...
//利用IMU计算位姿  //有两种情况会用到此函数：（a）视觉跟丢时用imu预测位姿。（b）imu模式下，恒速模型跟踪时提供位姿初始值。
...
void Tracking::Track()
{
    ...
    //获取Altas多图系统的active map
    //首先用pCurrentMap指针把active map拿出来，如果没有，那么就CreateMapInAtlas()或者mpSystem->ResetActiveMap()。
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                // 更新Local Mapping里的MapPoints
                CheckReplacedInLastFrame();

                // 如果没有帧间速度且当前帧IMU没有初始化，或者刚完成重定位，则与KeyFrame进行帧间匹配
                ...
                {
                    //先用恒速模型匹配，如果失败了只能还得TrackReferenceKeyFrame()
                    //融合imu的跟踪
                ...
                // 如果TrackReferenceKeyFrame()和TrackWithMotionModel()都没有搞定
                if (!bOK)
                {
                    // VI模型下跟踪失败，位姿丢失
            ...
            {
                // 刚跟丢，还可以抢救一下(mState==RECENTLY_LOST)
        ...
        // 6. (与local map的)中期数据匹配让pose更精确
        // 前面只是跟踪一帧得到初始位姿，这里搜索局部关键帧、局部地图点，和当前帧进行投影匹配，得到更多匹配的MapPoints后进行Pose优化，从而获得更准确的pose。
        // 前提是之前pose的初值求出来了，否则的话就重开一张图。
        ...
        if(pCurrentMap->isImuInitialized())
        {
            if(bOK)
            {
                //每过mnFramesToResetIMU帧就要ResetFrameIMU()一下
                if(mCurrentFrame.mnId==(mnLastRelocFrameId+mnFramesToResetIMU))
                {
        ...
        // 8. 保存恒速模型的速度
        // 也就是当前帧和上一帧的相对位姿，下一轮循环的时候用这个相对位姿乘以上一帧位姿来找最新帧的位姿
...
void Tracking::MonocularInitialization()
{
    ...
    if(!mbReadyToInitializate)
    {
        ...
        if(mCurrentFrame.mvKeys.size()>100)
        {
            ...
            mvbPrevMatched.resize(mCurrentFrame.mvKeysUn.size()); //mvKeyUn特征点的向量
            for(size_t i=0; i<mCurrentFrame.mvKeysUn.size(); i++)
                mvbPrevMatched[i]=mCurrentFrame.mvKeysUn[i].pt;

            //初始化匹配的特征点
            fill(mvIniMatches.begin(),mvIniMatches.end(),-1); //赋值都为-1
...
// 将3D点包装成MapPoint类型存入KeyFrame和Map中
void Tracking::CreateInitialMapMonocular()
{
    // 1. 按照关键帧进行Track的方法和运动模式恢复相机运动位姿的方法接近。首先求解当前帧的BOW向量。

    // 2. 再搜索当前帧和关键帧之间的关键点匹配关系，如果这个匹配关系小于15对的话，就Track失败了。 3. 接着将当前帧的位置假定到上一帧的位置那里
...
// 在tracking的最后会删除,跟踪过程中需要将将上一帧的MapPoints投影到当前帧可以缩小匹配范围，加快当前帧与上一帧进行特征点匹配
...
bool Tracking::TrackLocalMap()
{
    ...
    //Step 1：把当前帧的局部关键帧 mvpLocalKeyFrames 和局部地图点 mvpLocalMapPoints找出来
    UpdateLocalMap();
    ...
    // 优化前的位姿
    //---------------------------
    // Eigen::Matrix<float, 3, 1> exp1;
    
    // cout << exp1 << endl;
    // mCurrentFrame.mImuBias; //试imu偏差，不行就imu的位姿，速度相减求误差
    if (!mpAtlas->isImuInitialized())
        Optimizer::PoseOptimization(&mCurrentFrame);
    else
    {
...
void Tracking::CreateNewKeyFrame()
{
    ...
    // Step 2：对于双目或rgbd摄像头，为当前帧生成新的地图点；单目无操作
    ...
    // Step 3：插入关键帧
    // 关键帧插入到列表 mlNewKeyFrames中，等待local mapping线程
